...
const {
  ...
  countries,
  { name: '', has_parent: '', parent_id: '', is_active: '' },
...
const loadParents = async () => {
  try {
    const { data } = await countries.list({ has_parent: false, limit: 200, sort_by: 'id', sort_order: 'asc' })
    allParents.value = data?.data || []
  } catch { /* ignore */ }
}

onMounted(async () => {
  await Promise.all([reload(), loadParents()])
})

const parentMap = computed(() => {
  const m = {}
  for (const it of items.value) m[it.id] = localized(it.name)
  for (const it of allParents.value) m[it.id] = localized(it.name)
  return m
})
...
const cols = [
  ...
  { key: 'parent_id',  label: 'Родитель', width: '180px' },
...
const form = reactive({ name: {}, parent_id: '', is_active: true })
const saving = ref(false)

const reset = () => { form.name = {}; form.parent_id = ''; form.is_active = true; originalName.value = {} }
...
const submit = async () => {
  ...
  saving.value = false
  if (r.ok) { formOpen.value = false; loadParents() }
}
...
const showMeta = computed(() => {
  ...
  return [
    ...
    { key: 'parent',     label: tt('Родитель'),      value: r.parent_id ? (parentMap.value[r.parent_id] || `#${r.parent_id}`) : '—' },
...
<template>
  <div class="page">
    <PageHeader :title="tt('Страны')" :subtitle="tt('Страны / регионы / районы')">
      <template #actions>
    ...
    <div class="card">
      <div class="toolbar">
        ...
        <div class="field">
          <label class="field-label">{{ tt('Родитель') }}</label>
          <select class="select" v-model="filter.parent_id" @change="reload()">
            <option value="">{{ tt('Все') }}</option>
            <option v-for="p in allParents" :key="p.id" :value="p.id">{{ localized(p.name) }}</option>
          </select>
        </div>
        <div class="field">
    ...
    <div class="card">
      ...
      >
        ...
        <template #cell-parent_id="{ row }">
          <span v-if="row.parent_id">{{ parentMap[row.parent_id] || `#${row.parent_id}` }}</span>
    ...
    <AppModal v-model="formOpen" :title="editing ? tt('Изменить') : tt('Создать')">
      <form @submit.prevent="submit" class="form-grid">
        ...
        <div class="field">
          <label class="field-label">{{ tt('Родитель') }}</label>
          <select class="select" v-model="form.parent_id">
            <option value="">—</option>
            <option v-for="p in allParents" :key="p.id" :value="p.id">{{ localized(p.name) }}</option>
          </select>
        </div>
