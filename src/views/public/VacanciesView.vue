...
import { clientVacancies } from '@/api/endpoints'
...
const load = async (append = false) => {
  loading.value = true
  try {
    const { data } = await clientVacancies.list(buildParams())
...
<template>
  <div class="p-page">
    <div class="p-container">
      <div class="page-head">
        <div>
          <h1 class="p-h1">{{ tt('Вакансии') }}</h1>
          <p class="p-muted" style="margin: 4px 0 0;">{{ tt('Открытые предложения о работе по всей республике') }}</p>
        </div>
        <form class="page-search" @submit.prevent="submitSearch">
          <input v-model="search" type="search" class="p-input" :placeholder="tt('Поиск по вакансиям…')" />
          <button type="submit" class="p-btn p-btn-primary">{{ tt('Поиск') }}</button>
        </form>
      </div>

      <div class="body-grid">
        <FilterSidebar v-model="filter" @change="resetAndLoad" />
        ...
        <div class="results">
          ...
          <div v-else class="p-cards">
            <AdCard v-for="it in items" :key="it.id" :item="it" kind="vacancy" />
          </div>
