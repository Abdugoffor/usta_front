...
const cols = [
  ...
  { key: 'slug',       label: 'Ключ', sortable: true, width: '260px' },
  { key: 'name',       label: 'Перевод' },
...
const form = reactive({ slug: '', name: {}, is_active: true })
const saving = ref(false)

const reset = () => { form.slug = ''; form.name = {}; form.is_active = true; originalName.value = {} }
...
const submit = async () => {
  ...
  const name = editing.value
    ...
    : form.name
  const payload = { slug: form.slug, name, is_active: form.is_active }
  const r = editing.value
    ? await update(editing.value.id, payload)
    : await create(payload)
  saving.value = false
  if (r.ok) { formOpen.value = false; trStore.clearAll() }
}
...
<template>
  <div class="page">
    <PageHeader :title="tt('Переводы')" :subtitle="tt('Текстовые ключи интерфейса')">
      <template #actions>
    ...
    <div class="card">
      <div class="toolbar">
        <div class="field">
          <label class="field-label">{{ tt('Ключ') }}</label>
          <input class="input" v-model="filter.slug" :placeholder="tt('Поиск') + '…'" @keyup.enter="reload()" />
        </div>
        <div class="field">
          <label class="field-label">{{ tt('Перевод') }}</label>
    ...
    <div class="card">
      ...
      >
        <template #cell-slug="{ row }"><code>{{ row.slug }}</code></template>
    ...
    <AppModal v-model="formOpen" :title="editing ? tt('Изменить') : tt('Создать')">
      <form @submit.prevent="submit" class="form-grid">
        <div class="field full">
          <label class="field-label">{{ tt('Ключ') }} <span class="dim">(slug)</span></label>
          <input class="input" v-model="form.slug" required placeholder="Сохранить" />
        </div>
        <div class="full">
          <MultiLangInput
            v-model="form.name"
    ...
    <DetailModal v-model="showOpen" :title="tt('Просмотр') + ' — ' + (showRow?.slug || '')">
      <InfoList :items="showMeta" />
      <InfoList :items="showLangs" :title="tt('Перевод по языкам')" />
      <template #actions>
